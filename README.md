...
* `flags`: Flags are simply functions that run when an input tries to validate. See documentation on **[Built-it flags](#built-in-flags)** and **[customFlags](#customflags)**.
...
* Useful to validate usernames from a database; just pull a list of the usernames that are already taken and pass in an array. The array can be updated and Ideal Forms will respond accordingly. Just store the array in an external variable and pass in the variable.

...
You'll have to try out different values and widths depending on your container's width. Usually, adjusting the font should be enough or most cases, but if the font becomes too big or too small then it's a good idea to tweak the input's with and/or error width. The error is included in the width of the form, meaning that the container will become responsive as soon as the container can no longer wrap the the field including the error. This help to prevent the error from hiding outside the window if the form is aligned to the right and the window's width is small.
